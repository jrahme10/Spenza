...
const middle=`${account}${amountBlock}<TransactionCategoryPicker categories={data.categories} value={type==='transfer'?'Transfer':category} disabled={type==='transfer'} onSelect={setCategory} onAddCategory={addCategory} onEditCategory={editCategory} onDeleteCategory={deleteCategory}/><label>Date<input className="transactionDateInput" type="date" value={date} onChange={e=>{setDate(e.target.value);const sheet=e.currentTarget.closest('.sheet') as HTMLElement|null;const description=sheet?.querySelector('.transactionDescriptionInput') as HTMLInputElement|null;if(description){description.disabled=true;description.blur()}e.currentTarget.blur();sheet?.focus({preventScroll:true});requestAnimationFrame(()=>sheet?.focus({preventScroll:true}));window.setTimeout(()=>sheet?.focus({preventScroll:true}),80);window.setTimeout(()=>{sheet?.focus({preventScroll:true});if(description?.isConnected)description.disabled=false},320)}}/></label><div className="noteAttachmentLabel">Photos</div><NotePhotoPicker images={noteImages} onChange={setNoteImages}/><label>Description <small className="fieldHint">Optional</small><input className="transactionDescriptionInput" type="text" inputMode="text" autoComplete="off" autoCorrect="off" autoCapitalize="sentences" spellCheck={false} value={title} onChange={e=>setTitle(e.target.value)} enterKeyHint="done" onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();e.currentTarget.blur();const sheet=e.currentTarget.closest('.sheet') as HTMLElement|null;sheet?.focus({preventScroll:true})}}} placeholder={type==='income'?'Salary, refund...':type==='transfer'?'Move money...':'Optional description'}/></label><label>Note <small className="fieldHint">Optional</small><input className="transactionNoteInput" type="text" inputMode="text" autoComplete="off" value={note} onChange={e=>setNote(e.target.value)} enterKeyHint="done" onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();e.currentTarget.blur()}}} placeholder="Add a note (optional)"/></label>`
s=s.slice(0,start)+prefix+middle+save+s.slice(end)
writeFileSync(path,s)
console.log('Applied Add Transaction flow with Account above Amount and without automatic field advancement')
