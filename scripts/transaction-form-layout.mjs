...
const middle=`<TransactionCategoryPicker categories={data.categories} value={type==='transfer'?'Transfer':category} disabled={type==='transfer'} onSelect={setCategory} onAddCategory={(value)=>{const name=value.trim();if(!name)return;const existing=data.categories.find(c=>c.toLowerCase()===name.toLowerCase());if(existing){setCategory(existing);return}setData(d=>({...d,categories:[...d.categories,name]}));setCategory(name)}} onEditCategory={(oldValue,newValue)=>{const next=newValue.trim();if(!next||oldValue===next)return;const now=new Date().toISOString();setData(d=>({...d,categories:Array.from(new Set(d.categories.map(c=>c===oldValue?next:c))),transactions:d.transactions.map(t=>t.category===oldValue?{...t,category:next,updatedAt:now}:t),bills:d.bills.map(b=>b.category===oldValue?{...b,category:next,updatedAt:now}:b)}));if(category===oldValue)setCategory(next)}} onAdvance={()=>{const el=document.querySelector('.transactionAccountSelect') as HTMLSelectElement|null;el?.focus();el?.scrollIntoView({behavior:'smooth',block:'center'})}}/><label>Account<select className="transactionAccountSelect" value={walletId} onChange={e=>setWalletId(e.target.value)}><option value="">Select Account</option>{data.wallets.map(w=><option value={w.id} key={w.id}>{w.name} ({w.currency})</option>)}</select></label>{type==='transfer'&&<label>To Account<select value={toWalletId} onChange={e=>setToWalletId(e.target.value)}><option value="">Select Account</option>{data.wallets.filter(w=>w.id!==walletId).map(w=><option value={w.id} key={w.id}>{w.name} ({w.currency})</option>)}</select></label>}<label>Date<input type="date" value={date} onChange={e=>setDate(e.target.value)}/></label><div className="noteAttachmentLabel">Photos</div><NotePhotoPicker images={noteImages} onChange={setNoteImages}/><label>Description <small className="fieldHint">Optional</small><input value={title} onChange={e=>setTitle(e.target.value)} placeholder={type==='income'?'Salary, refund...':type==='transfer'?'Move money...':'Optional description'}/></label><label>Note <small className="fieldHint">Optional</small><input value={note} onChange={e=>setNote(e.target.value)} placeholder="Add a note (optional)"/></label>`
