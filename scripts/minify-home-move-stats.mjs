...
// Home toolbar order: account dropdown, wallet shortcut, Daily / Monthly / Calendar.
const period=`   <div className="filters refFilters periodFilters homePeriodFilters">{(['daily','monthly','yearly'] as InsightPeriod[]).map(p=><button key={p} className={homePeriod===p?'selected':''} onClick={()=>setHomePeriod(p)}>{p}</button>)}<button className={\`calendarFilterButton \${homePeriod==='custom'?'selected':''}\`} onClick={()=>setCalendarTarget('home')}><CalendarClock size={12}/> Calendar</button></div>\n`
...
console.log('Applied compact Home toolbar and moved Home statistics to Insights')
