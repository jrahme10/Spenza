...
export function serializePendingChange(
  data: SpenzaData,
  change: SyncChange,
): RemoteSyncRecord<RemotePayload> {
  ...
  if (!payload)
    throw new Error(`Missing local ${change.entityType} ${change.entityId} for pending upsert`)
  return {
    entityType: change.entityType,
    entityId: change.entityId,
    operation: 'upsert',
...
export function deserializeRemoteRecord(record: RemoteSyncRecord): RemoteSyncRecord<RemotePayload> {
  ...
  if (!record.payload || typeof record.payload !== 'object')
    throw new Error(`Remote ${record.entityType} ${record.entityId} is missing payload`)
