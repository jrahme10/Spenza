...
async function restoreFullCloudSnapshot(provider:SupabaseSyncProvider,local:SpenzaData,onProgress?:SyncOptions['onProgress'],signal?:AbortSignal):Promise<{data:SpenzaData;counts:RestoreCounts;serverTime:string}>{
  const records:RemoteSyncRecord[]=[];let cursor:string|undefined;let serverTime=epoch
  onProgress?.({phase:'downloading',processed:0,message:'Downloading cloud data…'})
  for(let page=0;page<20;page++){throwIfCancelled(signal);const pulled=await provider.pullChanges({since:epoch,limit:500,cursor},signal);throwIfCancelled(signal);records.push(...pulled.changes);serverTime=pulled.serverTime;onProgress?.({phase:'downloading',processed:records.length,message:`Downloaded ${records.length} cloud item${records.length===1?'':'s'}…`});if(!pulled.hasMore||!pulled.cursor)break;cursor=pulled.cursor}
  throwIfCancelled(signal)
...
export async function syncSupabaseIfAuthenticated(options:SyncOptions={}):Promise<SupabaseSyncResult>{
  ...
  try{
    ...
    let cursor:string|undefined;let serverTime=data.sync.lastSyncAt||epoch;let downloaded=0;progress?.({phase:'downloading',processed:0,message:'Checking for cloud updates…'})
    for(let page=0;page<20;page++){throwIfCancelled(signal);const pulled=await provider.pullChanges({since:data.sync.lastSyncAt,limit:500,cursor},signal);throwIfCancelled(signal);downloaded+=pulled.changes.length;for(const change of pulled.changes){throwIfCancelled(signal);data=applyRemote(data,change)}serverTime=pulled.serverTime;progress?.({phase:'downloading',processed:downloaded,message:downloaded?`Downloaded ${downloaded} cloud update${downloaded===1?'':'s'}…`:'No newer cloud changes.'});if(!pulled.hasMore||!pulled.cursor)break;cursor=pulled.cursor}
