...
const publicKey=()=>String((import.meta.env as Record<string,string|undefined>).VITE_VAPID_PUBLIC_KEY||'').trim()
...
export async function enableBillPush(){
 ...
 const vapidPublicKey=publicKey();if(!vapidPublicKey)throw new Error('Bill notifications are not configured yet.')
...
export async function disableBillPush(){
 ...
 if(subscription&&supabase&&ownerId){const {error}=await supabase.from('spenza_push_subscriptions').delete().eq('owner_id',ownerId).eq('endpoint',subscription.endpoint);if(error)throw error}
