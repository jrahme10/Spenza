...
export const syncManager={
  ...
  run():Promise<SupabaseSyncResult>{
    ...
    const task:Promise<SupabaseSyncResult>=syncSupabaseIfAuthenticated({onProgress:progress=>emit({status:'syncing',startedAt,progress})})
      .then(result=>{
      ...
      .catch((error):SupabaseSyncResult=>{const message=error instanceof Error?error.message:String(error);emit({status:'error',startedAt,finishedAt:Date.now(),error:message});return {status:'error',error:message}})
    active=task
    void task.finally(()=>{if(active===task)active=null})
    return task
  },
}
