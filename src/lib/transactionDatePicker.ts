...
function findDateInput(sheet: Element) {
  return Array.from(sheet.querySelectorAll<HTMLInputElement>('input[type="date"]')).find((input) =>
    input.closest('label')?.textContent?.trim().startsWith('Date'),
  )
}
...
function openDatePicker(input: HTMLInputElement, sheet: Element) {
  ...
  const render = () => {
    ...
    title.textContent = new Date(year, month - 1, 1).toLocaleDateString('en-US', {
      month: 'long',
      year: 'numeric',
    })
    grid.innerHTML = ''
    ...
    for (let i = 0; i < firstDay; i++) {
      ...
      spacer.className = 'transactionDateEmpty'
...
function prepare() {
  document.querySelectorAll(SHEET).forEach((sheet) => {
    ...
    const input = findDateInput(sheet)
    if (!input || input.dataset.transactionDatePicker === '1') return
    input.dataset.transactionDatePicker = '1'
    input.readOnly = true
    input.inputMode = 'none'
    const open = () => openDatePicker(input, sheet)
...
export function initTransactionDatePicker() {
  new MutationObserver(() => {
    prepare()
    if (!Array.from(document.querySelectorAll(SHEET)).some(transactionSheet)) datePickerCleanup?.()
