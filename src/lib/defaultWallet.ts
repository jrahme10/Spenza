...
function transactionSheet(el: Element) {
  return (
    el.matches('.sheet.refSheet') &&
    el.querySelector('h2')?.textContent?.trim() === 'Add Transaction'
  )
}
...
async function renderSettingsPreference() {
  ...
  try {
    ...
    if (!data || !settingsPage.isConnected || settingsPage.querySelector('.defaultWalletSetting'))
      return
...
export function initDefaultWallet() {
  const run = () => {
    void applyDefaultToAddTransaction()
    void renderSettingsPreference()
  }
