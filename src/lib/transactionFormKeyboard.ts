...
function accountSelect(sheet: Element) {
  const accountLabel = Array.from(sheet.querySelectorAll('label')).find((label) =>
...
function formatAmount(value: number, currency: Currency) {
  return currency === 'LBP'
    ? `${Math.round(value).toLocaleString()} LBP`
...
function evaluateAmountExpression(source: string): number | null {
  ...
  const consume = (value?: string) => {
    if (value && peek() !== value) throw new Error('Unexpected token')
    return text[index++]
  }
  const number = () => {
    const start = index
    let dots = 0
    while (index < text.length && /[0-9.]/.test(peek())) {
      if (peek() === '.' && ++dots > 1) throw new Error('Invalid number')
      index++
    }
...
function openAmountKeypad(input: HTMLInputElement, sheet: Element) {
  ...
  const onOutside = (event: PointerEvent) => {
    ...
    if (!target || pad.contains(target) || input.contains(target)) return
    closePad()
  }
  const closePad = () => {
    document.removeEventListener('pointerdown', onOutside, true)
    if (pad.isConnected) pad.remove()
    if (amountPadCleanup === closePad) amountPadCleanup = null
  }
  amountPadCleanup = closePad
  window.setTimeout(() => {
    if (pad.isConnected) document.addEventListener('pointerdown', onOutside, true)
  }, 0)
  ...
  const syncCurrencyButtons = () =>
    currencyButtons.forEach((button) =>
      button.classList.toggle('selected', button.dataset.currency === entryCurrency),
    )
  const showPreview = () => {
    ...
    resultEl.textContent =
      hasOperator && result !== null ? `= ${formatAmount(result, entryCurrency)}` : ''
    resultEl.classList.remove('error')
    if (result !== null && entryCurrency !== walletCurrency) {
      const converted = convertAmount(result, entryCurrency, walletCurrency)
      conversionEl.textContent = `Saved as ${formatAmount(converted, walletCurrency)} · 1 USD = ${usdToLbpRate.toLocaleString()} LBP`
    } else
      conversionEl.textContent =
        walletCurrency === entryCurrency ? `Account currency: ${walletCurrency}` : ''
  }
  const calculate = (commitToInput = true) => {
    const result = evaluateAmountExpression(draftValue)
    if (result === null) {
      resultEl.textContent = 'Invalid equation'
    ...
    if (commitToInput) {
      const converted = convertAmount(result, entryCurrency, walletCurrency)
      setReactInputValue(input, String(converted))
    }
    showPreview()
    return result
  }
  ...
  pad.querySelectorAll<HTMLButtonElement>('[data-key]').forEach((button) =>
    button.addEventListener('click', () => {
      ...
      if (key === 'back') {
        draftValue = draftValue.slice(0, -1)
        showPreview()
        return
      }
      if (key === '.') {
      ...
      if (/[+\-*/]/.test(key)) {
        ...
        if (/[+\-*/.]$/.test(draftValue) && !(key === '-' && /[*/+(]$/.test(draftValue)))
          draftValue = draftValue.slice(0, -1) + key
        else draftValue += key
        showPreview()
        return
      }
  ...
  const syncAccountAmount = () => {
    ...
    if (nextCurrency !== currentCurrency) {
      ...
      if (amount.value.trim() !== '' && Number.isFinite(currentAmount)) {
        setReactInputValue(
          amount,
          String(convertAmount(currentAmount, currentCurrency, nextCurrency)),
        )
      }
...
export function initTransactionFormKeyboard() {
  void loadData()
    .then((data) => {
      usdToLbpRate = data.usdToLbpRate || 89500
    })
    .catch(() => {})

  const observer = new MutationObserver(() => {
    const sheets = Array.from(document.querySelectorAll(transactionSheetSelector)).filter(
      isTransactionSheet,
    )
