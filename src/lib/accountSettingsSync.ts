...
function normalizePayload(value:unknown,fallback:SpenzaData):AccountSettingsPayload{
  ...
  const security=(raw.security&&typeof raw.security==='object'?raw.security:{}) as Partial<SyncedSecurity>
  return {
    categories:normalizeCategories(raw.categories??fallback.categories),
    usdToLbpRate:Number(raw.usdToLbpRate)>0?Number(raw.usdToLbpRate):fallback.usdToLbpRate||89500,
    security:{
    ...
    notificationReadIds:Array.isArray(raw.notificationReadIds)?[...new Set(raw.notificationReadIds.map(String))].sort():fallback.notificationReadIds,
    notificationDismissedIds:Array.isArray(raw.notificationDismissedIds)?[...new Set(raw.notificationDismissedIds.map(String))].sort():fallback.notificationDismissedIds,
  }
}
...
export async function syncAccountSettings(input?:SpenzaData):Promise<SpenzaData>{
  ...
  if(!row){
    ...
    const {error:upsertError}=await supabase.from('spenza_account_settings').upsert({owner_id:userId,payload:localPayload,changed_at:changedAt},{onConflict:'owner_id'})
  ...
  const remoteChanged=remoteFingerprint!==previousFingerprint&&(!previousChangedAt||row.changed_at>previousChangedAt)
