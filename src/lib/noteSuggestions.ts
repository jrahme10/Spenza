...
function suggestionOwnerKey(kind: SuggestionKind, ownerId?: string) {
  return `spenza-${kind}-suggestions:${ownerId || 'local-device'}`
}
...
function cleanNote(value?: string) {
  ...
  if (!note) return ''
  if (
    note === 'Added from scanned receipt' ||
    /^Receipt scanned in (USD|LBP)$/i.test(note) ||
    note === 'Paid from Bills'
  )
    return ''
  return note.endsWith(' · Paid from Bills')
    ? note.slice(0, -' · Paid from Bills'.length).trim()
    : note
}
...
async function loadSuggestions(input: HTMLInputElement, kind: SuggestionKind) {
  ...
  try {
    const [data, ownerId] = await Promise.all([
      loadData(),
      getSupabaseUserId().catch(() => undefined),
    ])
    ...
    try {
      remembered = JSON.parse(localStorage.getItem(storageKey) || '[]')
    } catch {
      remembered = []
    }
    if (kind === 'description') {
      const descriptions = sortedTransactions.map((item) => {
        ...
        if (
          item.type === 'expense' &&
          title.toLocaleLowerCase() === cleanDescription(item.category).toLocaleLowerCase()
        )
          return ''
        return title
      })
    ...
    try {
      localStorage.setItem(storageKey, JSON.stringify(suggestions.slice(0, 100)))
    } catch {}
    render()
  } catch {
    suggestions = []
