...
import {
  PullRemoteChangesRequest,
  PullRemoteChangesResult,
  PushRemoteChangesResult,
  RemoteSyncProvider,
  RemoteSyncRecord,
...
type TombstoneRow = {
  entity_type: 'wallet' | 'transaction' | 'bill'
  entity_id: string
  deleted_at: string
}
type PullRow = {
...
function toRemoteDelete(row: TombstoneRow): RemoteSyncRecord {
  return {
    ...
    operation: 'delete',
    changedAt: row.deleted_at,
  }
}
function toRemotePull(row: PullRow): RemoteSyncRecord {
  return {
    entityType: row.entity_type,
    entityId: row.entity_id,
    operation: row.operation,
    changedAt: row.changed_at,
    payload: row.operation === 'upsert' ? (row.payload ?? {}) : undefined,
  }
}
function syncRank(change: RemoteSyncRecord) {
  return change.operation === 'upsert'
    ? change.entityType === 'wallet'
      ? 0
...
function tableFor(entityType: RemoteSyncRecord['entityType']): EntityTable {
...
function shouldReplaceCurrent(current: RemoteSyncRecord | undefined, next: RemoteSyncRecord) {
  if (!current) return true
  if (next.changedAt > current.changedAt) return true
  if (next.changedAt < current.changedAt) return false
  return next.operation === 'delete' && current.operation !== 'delete'
}
function encodeCursor(record: RemoteSyncRecord) {
  return JSON.stringify({
    changedAt: record.changedAt,
    entityType: record.entityType,
    entityId: record.entityId,
  } satisfies PullCursor)
}
function decodeCursor(value?: string): PullCursor | undefined {
  if (!value) return undefined
  try {
    const parsed = JSON.parse(value) as Partial<PullCursor>
    if (
      typeof parsed.changedAt === 'string' &&
      ['wallet', 'transaction', 'bill'].includes(String(parsed.entityType)) &&
...
export class SupabaseSyncProvider implements RemoteSyncProvider {
  ...
  async pullChanges(
    request: PullRemoteChangesRequest,
    signal?: AbortSignal,
    ...
    return {
      ...
      hasMore,
      serverTime: await this.serverTime(signal),
    }
  }
  async pullCurrentRecords(
    targets: SyncTarget[],
    signal?: AbortSignal,
    ...
    for (const entityType of ['wallet', 'transaction', 'bill'] as const) {
      ...
      for (let start = 0; start < ids.length; start += LOOKUP_CHUNK_SIZE) {
        ...
        let entityQuery = this.supabase
          ...
          .select('id,payload,changed_at')
          .eq('owner_id', this.userId)
          .in('id', chunk)
        let tombstoneQuery = this.supabase
          .from('spenza_tombstones')
          .select('entity_type,entity_id,deleted_at')
          .eq('owner_id', this.userId)
          .eq('entity_type', entityType)
          .in('entity_id', chunk)
        if (signal) {
          entityQuery = entityQuery.abortSignal(signal)
          tombstoneQuery = tombstoneQuery.abortSignal(signal)
        }
        const [
          { data: entities, error: entityError },
          { data: tombstones, error: tombstoneError },
        ] = await Promise.all([entityQuery, tombstoneQuery])
        throwIfCancelled(signal)
        if (entityError) throw entityError
        if (tombstoneError) throw tombstoneError
        for (const row of (entities || []) as EntityRow[]) {
          const record = toRemoteEntity(entityType, row)
          const key = `${record.entityType}:${record.entityId}`
          if (shouldReplaceCurrent(current.get(key), record)) current.set(key, record)
        }
        for (const row of (tombstones || []) as TombstoneRow[]) {
          const record = toRemoteDelete(row)
          const key = `${record.entityType}:${record.entityId}`
          if (shouldReplaceCurrent(current.get(key), record)) current.set(key, record)
        }
      }
  ...
  async pushChanges(
    ...
    signal?: AbortSignal,
  ): Promise<PushRemoteChangesResult> {
    const accepted: RemoteSyncRecord[] = []
    const rejected: Array<{ change: RemoteSyncRecord; reason: string }> = []
    const ordered = [...changes].sort(
      (a, b) => syncRank(a) - syncRank(b) || a.changedAt.localeCompare(b.changedAt),
    )
    let processed = 0
    ...
    for (let start = 0; start < ordered.length; start += BATCH_SIZE) {
      ...
      const payload = batch.map((change) => ({
        entityType: change.entityType,
        entityId: change.entityId,
        operation: change.operation,
        changedAt: change.changedAt,
        payload: change.operation === 'upsert' ? (change.payload ?? {}) : null,
      }))
      ...
      for (let index = 0; index < batch.length; index++) {
        ...
        if (result?.accepted) accepted.push(change)
          ...
          rejected.push({
            change,
            reason: result?.reason || 'Cloud batch did not return a result for this change',
          })
        processed += 1
