...
function initialDesign():DesignPreference{try{const saved=JSON.parse(localStorage.getItem('spenza-design')||'{}');return {accent:typeof saved.accent==='string'?saved.accent:defaultDesign.accent,radius:['rounded','soft','square'].includes(saved.radius)?saved.radius:defaultDesign.radius}}catch{return defaultDesign}}
export function applyDesign(design:DesignPreference){const root=document.documentElement;root.style.setProperty('--accent',design.accent);root.dataset.radius=design.radius;delete root.dataset.density;localStorage.setItem('spenza-design',JSON.stringify(design))}
...
export default function ThemeControl(){
 ...
 return <>
  ...
  <div className="design-settings-card">
   ...
   <div className="designSettingBlock"><label>Accent color</label><label className="accentColorPicker"><input type="color" value={design.accent} onChange={e=>updateDesign('accent',e.target.value)}/><span className="accentColorPreview" style={{background:design.accent}}/><strong>{design.accent.toUpperCase()}</strong><span className="accentPickerHint">Choose color</span></label></div>
