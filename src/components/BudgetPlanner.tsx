...
export default function BudgetPlanner({wallet,transactions,categories,date}:Props){
 ...
 useEffect(()=>{let cancelled=false;const load=async()=>{if(!wallet){setBudget(empty);return}let local=empty;try{const raw=localStorage.getItem(key(wallet.id));local=normalize(raw?JSON.parse(raw):empty)}catch{}setBudget(local);const supabase=getSupabaseClient();if(!supabase)return;const {data:{session}}=await supabase.auth.getSession();if(!session||cancelled)return;const {data,error}=await supabase.from('spenza_budgets').select('payload').eq('owner_id',session.user.id).eq('id',wallet.id).maybeSingle();if(error||cancelled)return;if(data?.payload){const remote=normalize(data.payload);localStorage.setItem(key(wallet.id),JSON.stringify(remote));setBudget(remote);setSyncLabel('Synced')}};void load();return()=>{cancelled=true}},[wallet?.id])
 ...
 return <>
  <section className="budgetCard">
   <div className="budgetHead"><div><span className="budgetEyebrow"><Target size={15}/>Monthly Budget</span><h2>{budget.monthly?money(budget.monthly,wallet.currency):'Not set'}</h2><small>{wallet.name} · {new Date(`${month}-01T12:00:00`).toLocaleDateString('en-US',{month:'long',year:'numeric'})}{syncLabel?` · ${syncLabel}`:''}</small></div><button onClick={open} aria-label="Edit budget"><Pencil size={17}/></button></div>
