...
export default function SecuritySettings({data,setData}:Props){
  ...
  const submit=async()=>{
    ...
    if(mode==='setup'||mode==='change'){
      ...
      setData(d=>({...d,security:{...d.security,enabled:true,pinHash,salt,timeoutMinutes:d.security?.timeoutMinutes??0}}));close();return
    }
    if(!(await verify(pin))){setError('Incorrect PIN.');return}
    setData(d=>({...d,security:{enabled:false,timeoutMinutes:d.security?.timeoutMinutes??0,biometricEnabled:false}}));close()
  }

  const enableBiometric=async()=>{
    setBiometricBusy(true);setBiometricMessage('')
    try{
      const biometricCredentialId=await registerLocalBiometric()
      setData(d=>({...d,security:{...d.security,enabled:true,biometricEnabled:true,biometricCredentialId}}))
      setBiometricMessage('Biometric unlock is ready on this device.')
    }catch(err){setBiometricMessage(err instanceof Error?err.message:'Biometric setup could not be completed.')}
    finally{setBiometricBusy(false)}
  }
  const disableBiometric=()=>{setData(d=>({...d,security:{...d.security,biometricEnabled:false,biometricCredentialId:undefined}}));setBiometricMessage('Biometric unlock disabled.')}
  ...
  return <>
    ...
    {security.enabled&&<div className="securityOptions"><label>Auto-lock<select value={security.timeoutMinutes} onChange={e=>setData(d=>({...d,security:{...(d.security||{enabled:true}),enabled:true,timeoutMinutes:Number(e.target.value)}}))}><option value={0}>Immediately</option><option value={1}>After 1 minute</option><option value={5}>After 5 minutes</option><option value={15}>After 15 minutes</option></select></label><div><button onClick={()=>setMode('change')}>Change PIN</button><button className="securityDisable" onClick={()=>setMode('disable')}>Disable</button></div><div className="biometricSettings"><div><b>Face ID / Biometrics</b><small>{security.biometricEnabled?'Enabled on this device':biometricSupported?'Use your device authenticator to unlock':'Not available on this device/browser'}</small></div>{security.biometricEnabled?<button type="button" className="biometricDisable" onClick={disableBiometric}>Turn Off</button>:<button type="button" className="biometricEnable" disabled={!biometricSupported||biometricBusy} onClick={enableBiometric}>{biometricBusy?'Setting up…':'Enable'}</button>}</div>{biometricMessage&&<div className="biometricMessage">{biometricMessage}</div>}</div>}
