...
export default function CloudSyncSettings({data,setData}:Props){
  ...
  const runSync=async(successMessage='Everything is up to date.')=>{
    ...
    if(result.status==='signed-out'){setSignedInEmail('');setNeedsDataChoice(false);setStatus('signed-out');setMessage(result.error||'Your session expired. Sign in again.');return false}
