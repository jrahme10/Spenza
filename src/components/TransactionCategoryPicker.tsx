...
export default function TransactionCategoryPicker({categories,value,disabled,onSelect,onAddCategory,onAdvance}:Props){
 ...
 const saveNewCategory=()=>{
   ...
   if(!name)return
   const next=addType==='subcategory'&&addParent?`${addParent} > ${name}`:name
   onAddCategory(next)
   setAddOpen(false)
   setAddName('')
   setAddParent('')
 ...
 return <>
  ...
  {addOpen&&<div className="transactionCategorySheet categoryCreatorSheet" role="dialog" aria-modal="true" aria-label="Add category">
    <div className="transactionCategoryHandle"/>
    <div className="categoryCreatorHead"><b>Add Category</b><button type="button" onClick={()=>setAddOpen(false)} aria-label="Close"><X size={20}/></button></div>
