...
export default function AppLockGate({ data }: Props) {
  ...
  useEffect(() => {
    if (security?.enabled) setLocked(true)
  ...
  useEffect(() => {
    ...
    const onVisibility = () => {
      ...
      if ((security.timeoutMinutes ?? 0) === 0 || minutes >= (security.timeoutMinutes ?? 0))
        setLocked(true)
  ...
  const canUseBiometric =
    !!security?.biometricEnabled && !!security?.biometricCredentialId && biometricSupported
  ...
  return (
    <div className="appLockScreen">
      <div className="appLockCard">
        ...
        <p>
          {canUseBiometric
            ? 'Face ID / biometrics will open automatically. You can also use your PIN.'
            : 'Enter your PIN to unlock your finances.'}
        </p>
        {canUseBiometric && (
          <button className="biometricUnlock" onClick={unlockBiometric} disabled={biometricBusy}>
            <ShieldCheck />
            {biometricBusy ? 'Waiting for device…' : 'Try Face ID / Biometrics again'}
          </button>
        )}
        {canUseBiometric && (
