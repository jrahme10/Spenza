...
export default function NotePhotoPicker({ images, onChange, maxImages = 5 }: Props) {
  ...
  const addFiles = async (files?: FileList | null) => {
    ...
    try {
      const stored: string[] = []
      for (const file of picked) stored.push(await uploadNotePhoto(file))
  ...
  const openGallery = () => {
    setOpen(false)
    const input = galleryRef.current as PickerInput | null
    if (!input) return
    try {
  ...
  return (
    <div className="notePhotos compactPhotoPicker">
      <input
        ref={galleryRef}
      ...
      <button
        type="button"
        className={`photoAttachmentTrigger ${images.length ? 'hasPhotos' : ''}`}
      ...
      {open && (
        ...
        >
          <div className="photoChoiceSheet" onPointerDown={(e) => e.stopPropagation()}>
            ...
            <div className="photoChoiceHead">
              <div>
                <b>Add photo</b>
                <small>Choose a source</small>
              </div>
