...
export default function TransactionCalendar({
  ...
  return (
    <div className="calendarOverlay" onClick={onClose}>
      <div className="transactionCalendar" onClick={(e) => e.stopPropagation()}>
        <div className="calendarTitleRow">
          <div>
            <span className="eyebrow">FILTER BY DATE</span>
            <h2>{title}</h2>
          </div>
          <button className="calendarClose" onClick={onClose}>
            <X />
          </button>
        </div>
        ...
        <div className="calendarGrid">
          {cells.map((day, i) => {
            if (!day) return <span className="calendarEmpty" key={`e-${i}`} />
            ...
            return (
              <button
                key={date}
                className={`${marked ? 'hasTransactions ' : ''}${selected ? 'selectedDay' : ''}`}
                onClick={() => onSelect(date)}
              >
                <span>{day}</span>
                {marked && <i aria-label="Has transactions" />}
              </button>
            )
          })}
        </div>
        <div className="calendarLegend">
          <i /> Days with transactions
        </div>
      </div>
    </div>
  )
