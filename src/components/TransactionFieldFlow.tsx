...
type Targets = {
  ...
  categoryHost: HTMLElement
  accountSelect: HTMLSelectElement
  accountHost: HTMLElement
}

function isTransactionSheet(el: Element) {
...
function selectByLabel(sheet: HTMLElement, labelName: string) {
  const label = Array.from(sheet.querySelectorAll('label')).find((x) =>
    x.textContent?.trim().startsWith(labelName),
  )
...
function optionText(select: HTMLSelectElement) {
...
export default function TransactionFieldFlow() {
  ...
  useEffect(() => {
    ...
    if (firstField && firstField !== targets.accountHost)
      targets.sheet.insertBefore(targets.accountHost, firstField)
  ...
  const groups = useMemo(
    () =>
      targets
        ? categoryGroups(targets.categorySelect)
        : new Map<string, { raw: string; child: string }[]>(),
    [targets, version],
  )
  ...
  const categoryInput = createPortal(
    <input
      className="spenzaCategoryInput"
      value={optionText(targets.categorySelect)}
      readOnly
      inputMode="none"
      placeholder="Select category"
      aria-label="Select category"
      onClick={() => {
        if (!targets.categorySelect.disabled) {
          setAccountOpen(false)
          setExpanded('')
  ...
  const accountInput = createPortal(
    <input
      ref={accountInputRef}
      className="spenzaAccountInput"
      value={optionText(targets.accountSelect)}
      readOnly
      inputMode="none"
  ...
  return (
    <>
        ...
        createPortal(
          <>
            ...
            >
              ...
              <div className="categorySheetHead">
                <span />
                <b>Category</b>
                <button
              ...
              <div className="categoryOptions">
                {Array.from(groups.entries()).map(([parent, items]) => (
                  <div className="reactCategoryGroup" key={parent}>
                    <button
                      type="button"
                      className={
                        categoryValue === parent || categoryValue.startsWith(`${parent} > `)
                          ? 'selected'
                          : ''
                      }
                      onClick={() => chooseParent(parent, items)}
                    >
                      {parent}
                    </button>
                    {expanded === parent && items.filter((x) => x.child).length > 0 && (
                      <div className="reactSubcategoryGrid">
                        {items
                          .filter((x) => x.child)
                          .map((item) => (
                            <button
                              type="button"
                              className={categoryValue === item.raw ? 'selected' : ''}
                              key={item.raw}
                              onClick={() => chooseCategory(item.raw)}
                            >
                              {item.child}
                            </button>
                          ))}
      ...
      {accountOpen &&
        createPortal(
          <>
            <div className="spenzaPanelBackdrop" onClick={() => setAccountOpen(false)} />
            <div
              className="spenzaAccountSheet"
              role="dialog"
              aria-modal="true"
              aria-label="Select account"
              onClick={(e) => e.stopPropagation()}
            >
              ...
              <div className="accountOptions">
                {Array.from(targets.accountSelect.options)
                  .filter((o) => o.value)
                  .map((option) => {
                    ...
                    return (
                      <button
                        type="button"
                        className={accountValue === option.value ? 'selected' : ''}
                        key={option.value}
                ...
                {!Array.from(targets.accountSelect.options).some((o) => o.value) && (
                  <div className="accountEmpty">No accounts available.</div>
                )}
              </div>
            </div>
          </>,
