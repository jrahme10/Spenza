...
export default function InsightsScreen(){
 const {data}=useAppData();const [walletId,setWalletId]=useState(data.defaultWalletId||data.wallets[0]?.id||''),[category,setCategory]=useState('all'),[period,setPeriod]=useState<Period>('daily'),[anchor,setAnchor]=useState(today())
 const wallet=data.wallets.find(w=>w.id===walletId)||data.wallets[0]
 const filtered=useMemo(()=>wallet?data.transactions.filter(t=>t.walletId===wallet.id&&inPeriod(t.date,period,anchor)&&(category==='all'||t.category===category)):[],[data.transactions,wallet,category,period,anchor])
 const income=filtered.filter(t=>t.type==='income').reduce((s,t)=>s+t.amount,0),expense=filtered.filter(t=>t.type==='expense').reduce((s,t)=>s+t.amount,0),transfers=filtered.filter(t=>t.type==='transfer').reduce((s,t)=>s+t.amount,0)
 ...
 return <Screen title="Insights"><ScrollView contentContainerStyle={styles.wrap}>
  ...
  {wallet?<><View style={styles.stats}><Stat label="Expenses" value={money(expense,wallet.currency)}/><Stat label="Income" value={money(income,wallet.currency)}/></View><View style={styles.stats}><Stat label="Transactions" value={String(filtered.length)}/><Stat label="Transfers Out" value={money(transfers,wallet.currency)}/></View>
